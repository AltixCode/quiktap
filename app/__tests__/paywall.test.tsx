...
describe("Paywall", () => {
  ...
  it("expands a collapsed benefit on tap and shows its description", async () => {
    const { getByText, queryByText } = await renderWithProviders(<Paywall />);
    // The second benefit's body is collapsed by default (only the first is open).
    expect(queryByText(t("feat2Desc"))).toBeNull();
    await fireEvent.press(getByText(t("feat2Title")));
