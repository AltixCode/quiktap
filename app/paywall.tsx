...
export default function Paywall() {
  ...
  // Accordion stepper: exactly one benefit's body is visible at a time, and the
  // first one is open on arrival so the headline claim never needs a tap to read.
  // -1 means every row is collapsed (tapping the open row closes it).
      ...
      >
        ...
        <View style={{ marginTop: spacing["2xl"], gap: spacing.md }}>
          {benefits.map((benefit, index) => {
            const isOpen = index === expandedBenefit;
            return (
              <View
                key={benefit.title}
                style={{
              ...
              >
                <Pressable
                  ...
                  onPress={() => setExpandedBenefit(isOpen ? -1 : index)}
                  style={{
                    flexDirection: "row",
                    alignItems: "center",
                    gap: spacing.base,
